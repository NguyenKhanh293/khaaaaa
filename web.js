/* =========================
   DATABASE TẠM THỜI (localStorage)
========================= */
...
document.getElementById("btn-show-search").addEventListener("click", function() {
  renderSearchResults(customers);
  showScreen("search-menu");
});

document.getElementById("btn-back-from-search").addEventListener("click", function() {
  showScreen("main-menu");
});
...
document.getElementById("customer-form").addEventListener("submit", function(e) {
  ...
  const data = {
    ...
    city: document.getElementById("city-text").value.trim(), // input text
...
function editCustomer(index) {
  ...
  document.getElementById("company-phone").value = c.companyPhone;
  document.getElementById("zipcode").value = c.zipcode;
  document.getElementById("prefecture").value = c.prefecture;
  document.getElementById("city-text").value = c.city; // input text
  document.getElementById("chome").value = c.chome;
  document.getElementById("ban").value = c.ban;
  document.getElementById("go").value = c.go;
  document.getElementById("building").value = c.building;
  document.getElementById("payment-month").value = c.paymentMonth;
...
function renderSearchResults(list) {
  ...
  list.forEach((c,i)=>{
    html += `<tr>
      <td>${c.name}</td>
      <td>${c.phone}</td>
      <td>${c.prefecture} ${c.city} ${c.chome}-${c.ban}-${c.go} ${c.building}</td>
      <td>
...
/* Search by name */
document.getElementById("btn-search-name").addEventListener("click", function() {
...
document.getElementById("btn-search-address").addEventListener("click", function() {
  ...
  const city = document.getElementById("search-city-text").value.trim(); // input text
  const chome = document.getElementById("search-chome").value.trim();

  const results = customers.filter(c => {
    return (pref === "" || c.prefecture === pref) &&
           (city === "" || c.city === city) &&
           (chome === "" || c.chome === chome);
  });
  renderSearchResults(results);
...
/* Search by zipcode */
document.getElementById("btn-search-zipcode").addEventListener("click", function() {
  const zip = document.getElementById("search-zipcode").value.trim();
  const results = customers.filter(c => c.zipcode === zip);
  renderSearchResults(results);
});

/* Search by payment month */
document.getElementById("btn-search-payment").addEventListener("click", function() {
...
function lookupZipcode() {
  let zip = document.getElementById("zipcode").value.replace("-", "").trim();
  if(zip.length !== 7) return alert("Mã bưu điện phải đủ 7 chữ số");
  ...
  fetch(`https://zipcloud.ibsnet.co.jp/api/search?zipcode=${zip}`)
    ...
    .then(data => {
      if(data.results) {
        ...
        document.getElementById("city-text").value = r.address2 + r.address3; // input text
      } else {
...
window.onload = function() {
  ...
  // Populate months in search-payment select (24 tháng gần đây)
